...
internal class MultiTouchListener(
    ...
    private val mScaleGestureDetector: com.example.memej.textProperties.lib.ScaleGestureDetectorKotlin
        ...
        if (view is TextView) {
            if (onMultiTouchListener != null) {
                if (mOnPhotoEditorListener != null) {
                    if (isStart) mOnPhotoEditorListener.onStartViewChangeListener(ViewType.TEXT) else mOnPhotoEditorListener.onStopViewChangeListener(
                        ViewType.TEXT
                    )
                }
            } else {
                if (mOnPhotoEditorListener != null) {
                    if (isStart) mOnPhotoEditorListener.onStartViewChangeListener(ViewType.EMOJI) else mOnPhotoEditorListener.onStopViewChangeListener(
                        ViewType.EMOJI
                    )
                }
            }
            ...
            if (mOnPhotoEditorListener != null) {
                if (isStart) mOnPhotoEditorListener.onStartViewChangeListener(ViewType.IMAGE) else mOnPhotoEditorListener.onStopViewChangeListener(
                    ViewType.IMAGE
                )
            }
        }
    ...
    private inner class ScaleGestureListener :
        ScaleGestureDetectorKotlin.SimpleOnScaleGestureListener() {
        private var mPivotX = 0f
        private var mPivotY = 0f
        private val mPrevSpanVector = Vector2D()

        override fun onScaleBegin(view: View?, detector: ScaleGestureDetectorKotlin?): Boolean {
        ...
        override fun onScale(view: View?, detector: ScaleGestureDetectorKotlin?): Boolean {
    ...
    init {
        mScaleGestureDetector =
            ScaleGestureDetectorKotlin(ScaleGestureListener())
